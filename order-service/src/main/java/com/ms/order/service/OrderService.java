...
//	private WebClient webClient;
	...
	public OrderResponse placeOrder(OrderRequest orderRequest) {
		...
		// Micrometer Tracer is used to create a new custom spanId which can be used across microservices for tracing request logs with common span name
		Span inventoryServiceLookup = tracer.nextSpan().name("InventoryServiceLookup");
		inventoryServiceLookup.start();
		// Associate the span with a scope such that when the scope ends, the span closes.
		...
		try(Tracer.SpanInScope spanInScope =  tracer.withSpan(inventoryServiceLookup)) {
			...
			if (Arrays.stream(inventoryResponseArr).allMatch(inventoryResponse -> inventoryResponse.isInStock())) {
				...
				LOGGER.info("Order and List of OrderLineItem Saved to Repository");
				...
				List<OrderLineItemDtoResponse> orderLineItemDtoResponseList = orderDb.getOrderLineItemList().stream()
						.map(oli -> new OrderLineItemDtoResponse(oli.getOrderLineItemId(), oli.getSkuCode(), oli.getPrice(),
								oli.getQuantity()))
						.toList();
